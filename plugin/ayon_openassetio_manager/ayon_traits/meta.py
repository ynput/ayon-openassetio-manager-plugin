
"""
Trait definitions in the 'meta' namespace.

Traits related to metadata.
"""

# WARNING: This file is auto-generated by openassetio-traitgen, do not edit.

from typing import Union
import warnings

from openassetio.trait import TraitsData


class AnnotationTrait_v1:
    """
    A trait indicating an annotation.
    Usage: entity
    """
    kId = "Ayon:meta.Annotation"

    def __init__(self, traitsData):
        """
        Construct this trait view, wrapping the given data.

        @param traitsData @fqref{TraitsData}} "TraitsData" The target
        data that holds/will hold the traits properties.
        """
        self.__data = traitsData

    def isImbued(self):
        """
        Checks whether the data this trait has been applied to
        actually has this trait.
        @return `True` if the underlying data has this trait, `False`
        otherwise.
        """
        return self.isImbuedTo(self.__data)

    @classmethod
    def isImbuedTo(cls, traitsData):
        """
        Checks whether the given data actually has this trait.
        @param traitsData: Data to check for trait.
        @return `True` if the underlying data has this trait, `False`
        otherwise.
        """
        return traitsData.hasTrait(cls.kId)

    def imbue(self):
        """
        Adds this trait to the held data.

        If the data already has this trait, it is a no-op.
        """
        self.__data.addTrait(self.kId)

    @classmethod
    def imbueTo(cls, traitsData):
        """
        Adds this trait to the provided data.

        If the data already has this trait, it is a no-op.
        """
        traitsData.addTrait(cls.kId)

    
    def setAnnotation(self, annotation: str):
        """
        Sets the annotation property.

        Annotation.
        """
        if not type(annotation) is str:
            raise TypeError("annotation must be a 'str'.")
        self.__data.setTraitProperty(self.kId, "annotation", annotation)

    def getAnnotation(self, defaultValue: str=None) -> Union[str, None]:
        """
        Gets the value of the annotation property or the supplied default.

        Annotation.
        """
        value = self.__data.getTraitProperty(self.kId, "annotation")
        if value is None:
            return defaultValue

        if not type(value) is str:
            if defaultValue is None:
                raise TypeError(f"Invalid stored value type: '{type(value).__name__}' should be 'str'.")
            return defaultValue
        return value
        
    
class RelationshipTrait_v1:
    """
    A trait indicating a relationship.
    Usage: entity
    """
    kId = "Ayon:meta.Relationship"

    def __init__(self, traitsData):
        """
        Construct this trait view, wrapping the given data.

        @param traitsData @fqref{TraitsData}} "TraitsData" The target
        data that holds/will hold the traits properties.
        """
        self.__data = traitsData

    def isImbued(self):
        """
        Checks whether the data this trait has been applied to
        actually has this trait.
        @return `True` if the underlying data has this trait, `False`
        otherwise.
        """
        return self.isImbuedTo(self.__data)

    @classmethod
    def isImbuedTo(cls, traitsData):
        """
        Checks whether the given data actually has this trait.
        @param traitsData: Data to check for trait.
        @return `True` if the underlying data has this trait, `False`
        otherwise.
        """
        return traitsData.hasTrait(cls.kId)

    def imbue(self):
        """
        Adds this trait to the held data.

        If the data already has this trait, it is a no-op.
        """
        self.__data.addTrait(self.kId)

    @classmethod
    def imbueTo(cls, traitsData):
        """
        Adds this trait to the provided data.

        If the data already has this trait, it is a no-op.
        """
        traitsData.addTrait(cls.kId)

    
class XMPTrait_v1:
    """
    A trait indicating an XMP metadata.
    Usage: entity
    """
    kId = "Ayon:meta.XMP"

    def __init__(self, traitsData):
        """
        Construct this trait view, wrapping the given data.

        @param traitsData @fqref{TraitsData}} "TraitsData" The target
        data that holds/will hold the traits properties.
        """
        self.__data = traitsData

    def isImbued(self):
        """
        Checks whether the data this trait has been applied to
        actually has this trait.
        @return `True` if the underlying data has this trait, `False`
        otherwise.
        """
        return self.isImbuedTo(self.__data)

    @classmethod
    def isImbuedTo(cls, traitsData):
        """
        Checks whether the given data actually has this trait.
        @param traitsData: Data to check for trait.
        @return `True` if the underlying data has this trait, `False`
        otherwise.
        """
        return traitsData.hasTrait(cls.kId)

    def imbue(self):
        """
        Adds this trait to the held data.

        If the data already has this trait, it is a no-op.
        """
        self.__data.addTrait(self.kId)

    @classmethod
    def imbueTo(cls, traitsData):
        """
        Adds this trait to the provided data.

        If the data already has this trait, it is a no-op.
        """
        traitsData.addTrait(cls.kId)

    
    def setXmp(self, xmp: str):
        """
        Sets the xmp property.

        XMP metadata.
        """
        if not type(xmp) is str:
            raise TypeError("xmp must be a 'str'.")
        self.__data.setTraitProperty(self.kId, "xmp", xmp)

    def getXmp(self, defaultValue: str=None) -> Union[str, None]:
        """
        Gets the value of the xmp property or the supplied default.

        XMP metadata.
        """
        value = self.__data.getTraitProperty(self.kId, "xmp")
        if value is None:
            return defaultValue

        if not type(value) is str:
            if defaultValue is None:
                raise TypeError(f"Invalid stored value type: '{type(value).__name__}' should be 'str'.")
            return defaultValue
        return value
        
    
        
class AnnotationTrait(AnnotationTrait_v1):
    """
    A trait indicating an annotation.
    Usage: entity

    @deprecated Unversioned trait view classes are deprecated, please
    use AnnotationTrait_v1 explicitly.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        warnings.warn(
            "Unversioned trait view classes are deprecated. Please switch from"
            " AnnotationTrait to AnnotationTrait_v1.",
            DeprecationWarning,
            stacklevel=2
        )

    
        
class RelationshipTrait(RelationshipTrait_v1):
    """
    A trait indicating a relationship.
    Usage: entity

    @deprecated Unversioned trait view classes are deprecated, please
    use RelationshipTrait_v1 explicitly.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        warnings.warn(
            "Unversioned trait view classes are deprecated. Please switch from"
            " RelationshipTrait to RelationshipTrait_v1.",
            DeprecationWarning,
            stacklevel=2
        )

    
        
class XMPTrait(XMPTrait_v1):
    """
    A trait indicating an XMP metadata.
    Usage: entity

    @deprecated Unversioned trait view classes are deprecated, please
    use XMPTrait_v1 explicitly.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        warnings.warn(
            "Unversioned trait view classes are deprecated. Please switch from"
            " XMPTrait to XMPTrait_v1.",
            DeprecationWarning,
            stacklevel=2
        )

    